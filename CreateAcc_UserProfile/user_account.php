...
$sql = "SELECT * FROM `accounts` WHERE 1"; // Assuming user ID 1 for demonstration purposes
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["update"])) {
        ...
        $updateSql = "UPDATE `accounts` SET `firstName` = '{$firstName}', `lastName` = '{$lastName}', `email` = '{$email}' WHERE `nic` = '{$nic}'";
        
        if ($conn->query($updateSql) === TRUE) {
            echo "Profile updated successfully.";
            // Fetch the updated profile data
            $updatedResult = $conn->query("SELECT * FROM `accounts` WHERE `nic` = '{$nic}'");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["update"])) {
        ...
        $checkSql = "SELECT * FROM `accounts` WHERE `nic` = '{$nic}'";
        $result = $conn->query($checkSql);

        if ($result->num_rows > 0) {
            // User found, proceed with deletion
            $deleteSql = "DELETE FROM `accounts` WHERE `nic` = '{$nic}'";
